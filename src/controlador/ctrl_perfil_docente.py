...
class PerfilDocenteController(FletController):
    ...
    def cargar_datos_docente(self):
        ...
        row = ft.DataRow(cells=[
            ft.DataCell(ft.Text(f"{doc_info.pro_nombre_1} {doc_info.pro_nombre_2 or ''}".strip(), selectable=True)),
            ft.DataCell(ft.Text(f"{doc_info.pro_apellido_pat} {doc_info.pro_apellido_mat}", selectable=True)),
            ft.DataCell(ft.Text(doc_info.pro_rut, selectable=True)),
            ft.DataCell(ft.Text("Profesional PIE" if doc_info.pro_cargo == 1 else "Profesor Docente", selectable=True)),
            ft.DataCell(ft.Text(str(doc_info.num_encuestas) if hasattr(doc_info, 'num_encuestas') else "0", selectable=True)),
        ])
